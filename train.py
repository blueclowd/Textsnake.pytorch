...
from dataset.custom_text import CustomText
        ...
        if i % cfg.display_freq == 0:
            ...
            print(
                '({:d} / {:d}) - Loss: {:.4f} - tr_loss: {:.4f} - tcl_loss: {:.4f} - sin_loss: {:.4f} - cos_loss: {:.4f} - radii_loss: {:.4f}'.format(
                    i, len(train_loader), loss, tr_loss, tcl_loss, sin_loss,
                    cos_loss, radii_loss)
            )
...
def validation(model, valid_loader, criterion, epoch, logger):
    ...
    with torch.no_grad():
        ...
        for i, (img, train_mask, tr_mask, tcl_mask, radius_map, sin_map, cos_map, meta) in enumerate(valid_loader):
            ...
            tcl_losses.update(tcl_loss)
            sin_losses.update(sin_loss)
            cos_losses.update(cos_loss)
            radii_losses.update(radii_loss)
            ...
            if i % cfg.display_freq == 0:
                ...
                print(
                    'Validation: - Loss: {:.4f} - tr_loss: {:.4f} - tcl_loss: {:.4f} - sin_loss: {:.4f} - cos_loss: {:.4f} - radii_loss: {:.4f}'.format(
                        loss, tr_loss, tcl_loss, sin_loss,
                        cos_loss, radii_loss)
                )
...
def main():
    ...
    train_loader = data.DataLoader(trainset, batch_size=cfg.batch_size, shuffle=True, num_workers=cfg.num_workers, collate_fn=my_collate)
    if valset:
        val_loader = data.DataLoader(valset, batch_size=cfg.batch_size, shuffle=False, num_workers=cfg.num_workers, collate_fn=my_collate)
    else:
        valset = None
    ...
    for epoch in range(cfg.start_epoch, cfg.max_epoch):
        ...
        if valset:
            validation_loss = validation(model, val_loader, criterion, epoch, logger)
